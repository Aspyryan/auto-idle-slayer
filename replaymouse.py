...
def convert_to_pyautogui_script(recording):  
    ...
    output.write("from win32gui import FindWindow, GetWindowRect\n\n")  
    output.write("# Find the window handle and its rectangle\n")  
    output.write("window_handle = FindWindow(None, 'Idle Slayer')\n")  
    output.write("window_rect = GetWindowRect(window_handle)\n")  
    output.write("window_x, window_y, window_w, window_h = window_rect\n\n")  
    ...
    for i, step in enumerate(recording):  
        ...
        if step["action"] == "clicked":  
            last_click_time = step["_time"]  
            output.write(f"pyautogui.moveTo(window_x + {step['x']}, window_y + {step['y']}, duration=0.0)\n")  
        ...
        if step["action"] == "unclicked":  
            duration = step["_time"] - last_click_time if last_click_time else 0  
            output.write(f"pyautogui.moveTo(window_x + {step['x']}, window_y + {step['y']}, duration={duration})\n")  
            if step["button"] == "Button.right":  
